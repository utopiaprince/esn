...
{
    tran_cb_send_msg(M_TRAN_RXOK_EVENT, MSG_LOW_PRIO);
}
...
{
    tran_cb_send_msg(M_TRAN_RXOVR_EVENT, MSG_LOW_PRIO);
}
#endif
...
static void ack_timeout_cb(void *arg)
{
    ...
    if (tran_state.ack_received == FALSE)
    {
        ...
        tran_cb_send_msg(M_TRAN_ACK_TIMEOUT_EVENT, MSG_LOW_PRIO);
    }
    else
    {
        ;                                                   // 已经收到ACK
...
{
    csma_timer = NULL;
    tran_cb_send_msg(M_TRAN_CSMA_TIMEOUT_EVENT, MSG_LOW_PRIO);
}
...
void m_tran_event_handler(const osel_event_t *const pmsg)
{
    ...
    if (pmsg != NULL)
    {
        ...
        case M_TRAN_CSMA_TIMEOUT_EVENT:
            tran_operate_cca();
            break;

        case M_TRAN_RXOK_EVENT:
            tran_deal_rxok_event();
            break;

        case M_TRAN_RXOVR_EVENT:
...
#if TXOK_INT_SIMU_EN > 0
        case M_TRAN_TX_ERROR_EVENT:
            tran_deal_tx_error_event();
            break;
#endif

        case M_TRAN_TXUND_EVENT:
            tran_deal_tx_und_event();
            break;

        case  M_TRAN_ACK_TIMEOUT_EVENT:
